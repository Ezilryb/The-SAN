 ...
 *
 * FIX v3 :
 *  - filteredEntities() → méthode ordinaire appelée avec ()
 *    dans les templates. Alpine.js v3 CDN track les dépendances
 *    réactives (searchQuery, entityFilter) lors de l'évaluation
 *    de l'expression de template et re-rend automatiquement.
 *  - geoEntities / criticalCount → propriétés initialisées une
 *    fois dans init() (données statiques, pas de $watch nécessaire).
 *  - Double-init Leaflet → flag _mapInited levé IMMÉDIATEMENT
 *    dans le watcher avant le setTimeout.
 * ─────────────────────────────────────────────────────────────────
 */
...
window.palantirApp = function () {
  return {
    ...
    geoEntities:  [],   // entités avec coordonnées GPS
    criticalCount: 0,   // nombre d'alertes critiques
    ...
    /* ════════════════════════════════════════════════════════════
       ...
       Déclarée comme méthode ordinaire (pas getter, pas $watch).
       Appelée avec filteredEntities() dans les templates Alpine :
       Alpine évalue l'expression, accède à this.searchQuery et
       this.entityFilter → les enregistre comme dépendances →
       re-rend automatiquement quand elles changent.
       ════════════════════════════════════════════════════════════ */
    ...
    init() {
      ...
      this.$watch('currentView', view => {
        if (view === 'nexus' && !this._nexusInited) {
          this._nexusInited = true;           // lock immédiat → évite double-init
          this.nexusLoading = true;
          this.$nextTick(() => setTimeout(() => this._bootNexus(), 80));
        }
        if (view === 'map' && !this._mapInited) {
          this._mapInited = true;             // lock immédiat → évite "Map container already initialized"
    ...
    _bootNexus() {
      ...
      if (result) {
        ...
        this._nexusInited = false;  // reset le lock si l'init échoue
      }
    ...
    _bootMap() {
      ...
      if (!this._mapInst) this._mapInited = false;  // reset le lock si l'init échoue
